...
async function testModeAPI() {
  ...
  try {
    ...
    try {
      ...
      console.log('✅ Expected error for missing mode:', error.response?.data?.error || 'Network error');
    }
    console.log();
    ...
    try {
      ...
      console.log('✅ Expected error for invalid mode:', error.response?.data?.error || 'Network error');
    }
    console.log();
